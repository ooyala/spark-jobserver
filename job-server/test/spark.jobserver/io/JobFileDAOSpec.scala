...
class JobFileDAOSpec extends TestJarFinder with FunSpecLike with Matchers with BeforeAndAfter {
  ...
  var jarFile: File = new File(config.getString("spark.jobserver.filedao.rootdir"),
  ...
  before {
    FileUtils.deleteDirectory(new File(config.getString("spark.jobserver.filedao.rootdir")))
    dao = new JobFileDAO(config)
    jarFile.delete()
  }
